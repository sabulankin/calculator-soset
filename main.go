...
func main() {
	fmt.Println("Привет! Это калькулятор на Go.")
	fmt.Println("Поддерживаются скобки, отрицательные числа и музыкальные бонусы.")
	...
	for {
		...
		case "помощь":
			...
			fmt.Println("Примеры выражений: 2+2, (3+5)*2, -4/2")
			continue
		case "очистить":
			clearScreen()
			continue
